...
// A incrementer chaque fois que poids.bin change. Le fichier est
// mis en cache pour un an (voir _headers) : sans ce numero dans
// l'URL, les visiteurs garderaient l'ancienne version.
...
async function chargePoidsEntraines() {
  try {
    const rj = await fetch('poids.json?v=' + VERSION_POIDS);
    ...
    const rb = await fetch('poids.bin?v=' + VERSION_POIDS);
